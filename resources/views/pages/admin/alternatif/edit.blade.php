...
<form action="{{ route('alternatif.update', $alternatif->id) }}" method="POST">
    ...
    <fieldset disabled>
        <div class="row">
            <div class="mb-3 col-lg-6">
                <label for="nameSelected" class="form-label">Nama yang dipilih</label>
                <input type="text" class="form-control" id="nameSelected" value="{{ old('name', $alternatives->name) }}"
                    readonly required>
                @error('name')
            ...
            <div class="mb-3 col-lg-6">
                <label for="kelasSelected" class="form-label">Kelas</label>
                <input type="text" class="form-control" id="kelasSelected"
        ...
        @foreach ($newCriterias as $value)
            <div class="mb-3">
                ...
                <input type="text" id="{{ str_replace(' ', '', $value->name) }}"
                    class="form-control @error('new_alternative_value') is-invalid @enderror" name="new_alternative_value[]"
                    placeholder="Enter the value"
                    onkeypress="return (event.charCode != 8 && event.charCode == 0 || (event.charCode >= 48 && event.charCode <= 57) || event.charCode == 46)"
    ...
    <a href="{{ route('alternatif.index') }}" class="btn btn-danger mb-3">Cancel</a>
</form>
